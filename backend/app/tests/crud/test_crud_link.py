...
# To potentially access other settings if needed
...
def temp_cookie_structure(tmp_path: Path, monkeypatch) -> Path:
    ...
    monkeypatch.setattr(
        crud_link_module, "USER_COOKIES_BASE_DIR_NAME", user_cookies_base_name
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "my_cookie.txt",
            {"exists": True, "isfile": True, "isabs": False, "commonpath_match": True},
            "my_cookie.txt",
            None,
        ),
        (
            "subdir/my_cookie.txt",
            {"exists": True, "isfile": True, "isabs": False, "commonpath_match": True},
            os.path.join("subdir", "my_cookie.txt"),
            None,
        ),
        (None, {}, None, None),  # Path is None
        # Invalid cases - ValueErrors
        ("/abs/path/cookie.txt", {"isabs": True}, None, ValueError),  # Absolute path
        # Directory traversal (normpath results in ..)
        (
            "../secrets.txt",
            {"isabs": False, "normpath_traversal": True},
            None,
            ValueError,
        ),
        (
            "outside_cookie.txt",
            {"exists": True, "isfile": True, "isabs": False, "commonpath_match": False},
            None,
            ValueError,
        ),
        # Resolves outside base dir
        (
            "nonexistent.txt",
            {"exists": False, "isabs": False},
            None,
            ValueError,
        ),  # File does not exist
        (
            "is_a_dir",
            {"exists": True, "isfile": False, "isabs": False},
            None,
            ValueError,
        ),
        # Path is a directory
...
def test_validate_and_normalize_cookies_path(
    ...
    # This sets up PROJECT_ROOT and USER_COOKIES_BASE_DIR_NAME
    temp_cookie_structure: Path,
):
    ...
    # Create dummy files/dirs based on current param for os.path.exists/isfile
    # if needed by the scenario
    ...
    with mock.patch(
        ...
        def dynamic_exists(path_arg):
            # Check if path_arg corresponds to the input file within the mocked
            # structure
            if user_path_input and str(cookies_base_dir_path / user_path_input) in str(
                path_arg
            ):
        ...
        if expect_error:
            with pytest.raises(expect_error):
                crud_link_obj_for_test._validate_and_normalize_cookies_path(
                    user_path_input
                )
        else:
            result = crud_link_obj_for_test._validate_and_normalize_cookies_path(
                user_path_input
            )
...
@pytest.mark.asyncio
async def test_create_link_with_valid_cookies_path(
    db: AsyncSession, temp_cookie_structure: Path
):
    ...
    # We are not mocking _validate_and_normalize_cookies_path here, letting it
    # run with patched os.path
    with mock.patch("os.path.exists", return_value=True), mock.patch(
        "os.path.isfile", return_value=True
    ), mock.patch("os.path.isabs", return_value=False), mock.patch(
        "os.path.commonpath", return_value=str(temp_cookie_structure.parent)
    ):  # mock commonpath to match project_root
        created_link = await crud_link_instance.create(db=db, obj_in=link_in)
...
async def test_create_link_cookies_path_none(db: AsyncSession):
    link_in = LinkCreate(
        url="http://nocookie.com", name="No Cookie Link", cookies_path=None
    )
    created_link = await crud_link_instance.create(db=db, obj_in=link_in)
...
@pytest.mark.asyncio
async def test_create_link_invalid_cookies_path_value_error(
    db: AsyncSession, temp_cookie_structure: Path
):
    link_in = LinkCreate(
        url="http://invalidcookie.com",
        name="Invalid Cookie Link",
        # This should cause _validate to fail
        cookies_path="/absolute/path/not_allowed.txt",
    )
    # Let _validate_and_normalize_cookies_path run, it should raise ValueError
    # due to absolute path
    with mock.patch(
        "os.path.isabs", return_value=True
    ):  # Mock isabs to trigger the error
...
@pytest.mark.asyncio
async def test_update_link_with_valid_cookies_path(
    db: AsyncSession, test_link: Link, temp_cookie_structure: Path
):
    ...
    update_data = LinkUpdate(
        cookies_path=cookie_file_path_relative, name="Updated Name CP"
    )
    ...
    with mock.patch("os.path.exists", return_value=True), mock.patch(
        ...
        updated_link = await crud_link_instance.update(
            db=db, db_obj=test_link, obj_in=update_data
        )
...
@pytest.mark.asyncio
async def test_update_link_cookies_path_to_none(
    db: AsyncSession, test_link: Link, temp_cookie_structure: Path
):
    # First, set a cookie path
    initial_cookie_file = "user_initial/cookie.txt"
    (temp_cookie_structure / initial_cookie_file).parent.mkdir(
    ...
    with mock.patch("os.path.exists", return_value=True), mock.patch(
        ...
        link_with_cookie = await crud_link_instance.update(
            db=db, db_obj=test_link, obj_in=LinkUpdate(cookies_path=initial_cookie_file)
        )
    assert link_with_cookie.cookies_path == initial_cookie_file

    # Now update to None
    update_to_none_data = LinkUpdate(cookies_path=None)
    updated_link_no_cookie = await crud_link_instance.update(
        db=db, db_obj=link_with_cookie, obj_in=update_to_none_data
    )
    assert updated_link_no_cookie.cookies_path is None


@pytest.mark.asyncio
async def test_update_link_invalid_cookies_path_value_error(
    db: AsyncSession, test_link: Link, temp_cookie_structure: Path
):
    update_data = LinkUpdate(cookies_path="../attempt_traversal.txt")
    # Mock normpath to simulate traversal
    with mock.patch("os.path.normpath", return_value="../attempt_traversal.txt"):
        with pytest.raises(ValueError) as excinfo:
...
async def test_get_by_url_non_existent(db: AsyncSession):
    fetched_link = await crud_link_instance.get_by_url(
        db=db, url="http://nonexistenturl.com"
    )
...
async def test_get_enabled_links(db: AsyncSession):
    await crud_link_instance.create(
        db=db,
        obj_in=LinkCreate(
            url="http://enabled1.com",
            name="Enabled 1",
            is_enabled=True,
    ...
    creator_enabled = await crud_link_instance.get_enabled_links(
        db=db, link_type=LinkType.CREATOR
    )
    ...
    live_enabled = await crud_link_instance.get_enabled_links(
        db=db, link_type=LinkType.LIVE
    )
    ...
    other_type_enabled = await crud_link_instance.get_enabled_links(
        db=db, link_type=LinkType.OTHER
    )
...
async def test_update_status(db: AsyncSession, test_link: Link):
    ...
    link_idle_success = await crud_link_instance.update_status(
        db=db, db_obj=test_link, status=LinkStatus.IDLE, is_success=True
    )
    assert link_idle_success.status == LinkStatus.IDLE
    assert link_idle_success.error_message is None
    assert (
        link_idle_success.last_checked_at > test_link.last_checked_at
        if test_link.last_checked_at
        else True
    )
    assert (
        link_idle_success.last_success_at > test_link.last_success_at
        if test_link.last_success_at
        else True
    )
    ...
    link_downloading = await crud_link_instance.update_status(
        db=db, db_obj=link_idle_success, status=LinkStatus.DOWNLOADING, is_success=False
    )
    ...
    link_error = await crud_link_instance.update_status(
        db=db,
        db_obj=link_downloading,
