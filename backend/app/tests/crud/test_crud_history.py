...
# To create prerequisite Link objects
from app.crud.crud_link import link as crud_link
# from app.tests.conftest import TestSessionFactory  # Not used directly
...
async def test_create_log_all_fields(db: AsyncSession, create_test_link: Link):
    ...
    assert (datetime.now(timezone.utc) - created_log.timestamp).total_seconds() < 5


@pytest.mark.asyncio
async def test_create_log_optional_fields_omitted(
    db: AsyncSession, create_test_link: Link
):
    ...
    assert created_log.status == log_data["status"]
    assert created_log.error_message == log_data["error_message"]
    # Should default to None or empty list based on model
    assert created_log.downloaded_files is None
...
async def test_get_multi_by_link(db: AsyncSession, create_test_link: Link):
    ...
    other_link_in = LinkCreate(
        url="http://otherlink.com", name="Other Link", link_type=LinkType.OTHER
    )
    ...
    # log1_l1 was F841
    await crud_history_log.create_log(
        db=db, link_id=link1_id, status=HistoryStatus.SUCCESS, details={"order": 1}
    )
    await asyncio.sleep(0.01)  # Ensure timestamp difference
    log2_l1 = await crud_history_log.create_log(
        db=db,
        link_id=link1_id,
        status=HistoryStatus.FAILURE,
        error_message="err1",
    ...
    await crud_history_log.create_log(
        db=db, link_id=other_link_id, status=HistoryStatus.SUCCESS
    )

    # Test fetching all logs for link1
    logs_l1_all = await crud_history_log.get_multi_by_link(
        db=db, link_id=link1_id, limit=10
    )
    assert len(logs_l1_all) == 3
    assert [log.details["order"] for log in logs_l1_all] == [
        3,
        2,
        1,
    ...
    logs_l1_paginated = await crud_history_log.get_multi_by_link(
        db=db, link_id=link1_id, skip=1, limit=1
    )
    assert len(logs_l1_paginated) == 1
    assert logs_l1_paginated[0].id == log2_l1.id

    # Test fetching logs for a link with no history
    no_history_link_in = LinkCreate(
        url="http://nohistory.com", name="No History Link", link_type=LinkType.OTHER
    )
    no_history_link = await crud_link.create(db=db, obj_in=no_history_link_in)
    logs_no_history = await crud_history_log.get_multi_by_link(
        db=db, link_id=no_history_link.id
    )
...
async def test_remove_by_link(db: AsyncSession, create_test_link: Link):
    ...
    link1_id = create_test_link.id
    other_link_in = LinkCreate(
        url="http://otherlinkremove.com",
        name="Other Link Remove",
    ...
    await crud_history_log.create_log(
        db=db, link_id=link1_id, status=HistoryStatus.SUCCESS
    )
    await crud_history_log.create_log(
        db=db, link_id=link1_id, status=HistoryStatus.SUCCESS
    )
    ...
    log_other_link = await crud_history_log.create_log(
        db=db, link_id=other_link_id, status=HistoryStatus.SUCCESS
    )
    ...
    logs_l1_after_delete = await crud_history_log.get_multi_by_link(
        db=db, link_id=link1_id
    )
    assert len(logs_l1_after_delete) == 0

    # Verify logs for other_link are not affected
    logs_other_link_after_delete = await crud_history_log.get_multi_by_link(
        db=db, link_id=other_link_id
    )
    ...
    no_history_link_in = LinkCreate(
        url="http://nohistoryremove.com",
        name="No History Link Remove",
        link_type=LinkType.OTHER,
    )
    no_history_link = await crud_link.create(db=db, obj_in=no_history_link_in)
    deleted_count_no_history = await crud_history_log.remove_by_link(
        db=db, link_id=no_history_link.id
    )
...
async def test_crudbase_get(db: AsyncSession, create_test_link: Link):
    ...
    log_created = await crud_history_log.create_log(
        db=db, link_id=create_test_link.id, status=HistoryStatus.SUCCESS
    )
...
async def test_crudbase_get_multi(db: AsyncSession, create_test_link: Link):
    ...
    log1 = await crud_history_log.create_log(
        db=db,
        link_id=create_test_link.id,
        status=HistoryStatus.SUCCESS,
        details={"order": 1},
    )
    # ensure timestamp difference for ordering if default order is applied
    await asyncio.sleep(0.01)
    log2 = await crud_history_log.create_log(
        db=db,
        link_id=create_test_link.id,
        status=HistoryStatus.FAILURE,
        details={"order": 2},
    ...
    # Test get_multi without specific ordering (relies on insertion order or
    # PK if no default in model)
    all_logs = await crud_history_log.get_multi(db=db, limit=10)
    # Could be more if other tests ran without full isolation
    ...
    paginated_logs = await crud_history_log.get_multi(
        db=db, skip=0, limit=1, order_by=[HistoryLog.timestamp.asc()]
    )
    ...
    # For HistoryLog, get_multi_by_link orders by timestamp.
    # CRUDBase.get_multi has an order_by param.

    # Test with explicit ordering (ascending by timestamp)
    ordered_logs_asc = await crud_history_log.get_multi(
        db=db, order_by=[HistoryLog.timestamp.asc()], limit=2
    )
    if (
        len(ordered_logs_asc) == 2
        and ordered_logs_asc[0].id == log1.id
        and ordered_logs_asc[1].id == log2.id
    ):
        # This order is correct if log1 was created before log2
        pass
    elif (
        len(ordered_logs_asc) == 2
        and ordered_logs_asc[0].id == log2.id
        and ordered_logs_asc[1].id == log1.id
    ):
        # This means log2 was created before log1, which is opposite to the
        # code
        pytest.fail(
            "Log creation order seems to be problematic for timestamp ordering test."
        )
...
async def test_crudbase_create(db: AsyncSession, create_test_link: Link):
    ...
    # We can add a direct call for completeness if needed, using
    # HistoryLogCreate.
...
async def test_crudbase_update(db: AsyncSession, create_test_link: Link):
    ...
    log_created = await crud_history_log.create_log(
        ...
        link_id=create_test_link.id,
        status=HistoryStatus.SUCCESS,
        details={"original_detail": "value1"},
    )

    original_timestamp = log_created.timestamp
    # Ensure updated_at will be different if CRUDBase.update handles it
    await asyncio.sleep(0.01)

    update_data = {
        "status": HistoryStatus.FAILURE,
        "error_message": "Updated error message",
        # Completely replaces details
        "details": {"original_detail": "value1", "new_detail": "value2"},
    }
    ...
    log_updated = await crud_history_log.update(
        db=db, db_obj=log_created, obj_in=update_data
    )
    ...
    # assert log_updated.updated_at > original_timestamp (or if it was None
    # before)
