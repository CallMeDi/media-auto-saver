...
from app.models.password_reset import PasswordResetToken # PasswordResetTokenCreate removed

# To create prerequisite User objects
# from app.crud.crud_user import user as crud_user # Unused
from app.crud.crud_password_reset import password_reset_token as crud_pr_token_instance

# For patching module-level functions
from app.crud import crud_password_reset as crud_pr_module
...
async def test_user(db: AsyncSession) -> User:
    ...
    user_in = UserCreate(
        # Unique username
        username=f"testuser_{datetime.now().strftime('%Y%m%d%H%M%S%f')}",
        # Unique email
        email=f"test_{datetime.now().strftime('%Y%m%d%H%M%S%f')}@example.com",
        password="testPassword123",
    )
    # Use the actual crud_user.create method
    # If crud_user.create is synchronous, adapt or use a direct User model creation
    # For this example, assume crud_user.create exists and is async, or use a
    # simplified creation:
...
@pytest.mark.asyncio
@mock.patch.object(
    model_pr_module, "generate_reset_token", return_value=MOCKED_TOKEN_STR
)
@mock.patch.object(
    model_pr_module, "calculate_expiry_date", return_value=MOCKED_FUTURE_DATETIME
)
async def test_create_reset_token(
    mock_generate_token, mock_calculate_expiry, db: AsyncSession, test_user: User
):
    """Test creating a password reset token."""
    created_token_obj = await crud_pr_token_instance.create_reset_token(
...
async def test_get_by_token(
    ...
    original_token_obj = await crud_pr_token_instance.create_reset_token(
        db=db, user_id=test_user.id
    )

    # Fetch it
    fetched_token_obj = await crud_pr_token_instance.get_by_token(
        db=db, token=MOCKED_TOKEN_STR
    )
    ...
    non_existent_token_obj = await crud_pr_token_instance.get_by_token(
        db=db, token="non_existent_fake_token"
    )
    assert non_existent_token_obj is None


@pytest.mark.asyncio
# Patch datetime within the model module
@mock.patch.object(model_pr_module, "datetime")
async def test_use_token(
    ...
    # Create a token (use default mocks for generate/calculate for simplicity
    # here, or re-patch)
    with mock.patch.object(
        model_pr_module, "generate_reset_token", return_value="another_token"
    ), mock.patch.object(
        model_pr_module, "calculate_expiry_date", return_value=MOCKED_FUTURE_DATETIME
    ):
        token_to_use = await crud_pr_token_instance.create_reset_token(
            db=db, user_id=test_user.id
        )
    ...
    used_token_obj = await crud_pr_token_instance.use_token(
        db=db, token_obj=token_to_use
    )
    ...
    # Patch datetime in crud_pr_module
    with mock.patch.object(crud_pr_module, "datetime") as mock_crud_datetime:
        ...
        token_to_use_again = await db.get(PasswordResetToken, token_to_use.id)
        refreshed_used_token_obj = await crud_pr_token_instance.use_token(
            db=db, token_obj=token_to_use_again
        )
...
async def test_is_token_valid(db: AsyncSession, test_user: User):
    ...
    with mock.patch.object(
        model_pr_module, "generate_reset_token", return_value="valid_token"
    ), mock.patch.object(
        model_pr_module, "calculate_expiry_date", return_value=MOCKED_FUTURE_DATETIME
    ):
        fresh_token = await crud_pr_token_instance.create_reset_token(
            db=db, user_id=test_user.id
        )
    assert crud_pr_token_instance.is_token_valid(fresh_token) is True

    # 2. Used token
    # This also expires it
    used_token = await crud_pr_token_instance.use_token(db=db, token_obj=fresh_token)
    # Should be false because it's used AND expired by use_token
    assert crud_pr_token_instance.is_token_valid(used_token) is False

    # 3. Expired token (not used)
    with mock.patch.object(
        model_pr_module, "generate_reset_token", return_value="expired_token_str"
    ), mock.patch.object(
        model_pr_module, "calculate_expiry_date", return_value=MOCKED_PAST_DATETIME
    ):  # Expires in the past
        expired_token = await crud_pr_token_instance.create_reset_token(
            db=db, user_id=test_user.id
        )
    ...
    # Let's create one manually that's used, and has a past expiry different
    # from 'now'
    ...
    # SQLModel/SQLAlchemy should handle this. We are mostly testing the
    # `is_token_valid` logic.

    token_for_tz_test = PasswordResetToken(
        token="tz_test_token",
        user_id=test_user.id,
        # Create a NAIVE datetime but in UTC
    ...
    await db.commit()
    # token_for_tz_test.expires_at might be naive or aware here
    await db.refresh(token_for_tz_test)
    ...
    mock_token_naive_expiry.expires_at = datetime.utcnow() + timedelta(
        hours=1
    )  # Naive datetime

    # Patch datetime.now in crud_pr_module
    with mock.patch.object(crud_pr_module, "datetime") as mock_crud_dt_for_is_valid:
        # Make "now" well before the naive token's expiry
        mock_crud_dt_for_is_valid.now.return_value = datetime.utcnow().replace(
            tzinfo=timezone.utc
