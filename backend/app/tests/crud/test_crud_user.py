...
# from typing import Optional, Dict, Any # All unused
...
# from app.crud import crud_user as crud_user_module # Unused

# For patching security functions
from app.core import security as core_security_module
...
async def test_get_by_username_existing(db: AsyncSession, create_raw_user_in_db: User):
    ...
    fetched_user = await crud_user_instance.get_by_username(
        db=db, username=raw_user.username
    )
...
async def test_get_by_username_non_existent(db: AsyncSession):
    ...
    fetched_user = await crud_user_instance.get_by_username(
        db=db, username="nonexistentuser"
    )
...
@pytest.mark.asyncio
@mock.patch.object(
    core_security_module, "get_password_hash", return_value=MOCKED_HASHED_PASSWORD
)
...
@pytest.mark.asyncio
async def test_update_user_no_password_change(
    db: AsyncSession, create_raw_user_in_db: User
):
    ...
    updated_user = await crud_user_instance.update(
        db=db, db_obj=raw_user, obj_in=update_data
    )
...
@pytest.mark.asyncio
@mock.patch.object(
    core_security_module, "get_password_hash", return_value=NEW_MOCKED_HASHED_PASSWORD
)
async def test_update_user_password_only(
    mock_get_password_hash, db: AsyncSession, create_raw_user_in_db: User
):
...
async def test_update_user_password_and_other_attributes(
    mock_get_password_hash, db: AsyncSession, create_raw_user_in_db: User
):
    ...
    updated_user = await crud_user_instance.update(
        db=db, db_obj=raw_user, obj_in=update_data_dict
    )
...
async def test_authenticate_correct_credentials(db: AsyncSession):
    ...
    with mock.patch.object(
        ...
        user_in = UserCreate(
            username=username, email="auth_correct@example.com", password=plain_password
        )
        await crud_user_instance.create(db=db, obj_in=user_in)

    # Mock verify_password to return True for these credentials
    with mock.patch.object(
        core_security_module, "verify_password", return_value=True
    ) as mock_verify:
        authenticated_user = await crud_user_instance.authenticate(
...
async def test_authenticate_incorrect_password(db: AsyncSession):
    ...
    with mock.patch.object(
        ...
        user_in = UserCreate(
            username=username,
            email="auth_incorrect_pw@example.com",
            password=correct_plain_password,
        )
        await crud_user_instance.create(db=db, obj_in=user_in)

    # Mock verify_password to return False
    with mock.patch.object(
        core_security_module, "verify_password", return_value=False
    ) as mock_verify:
        authenticated_user = await crud_user_instance.authenticate(
            db=db, username=username, password=incorrect_plain_password
        )

    assert authenticated_user is None
    mock_verify.assert_called_once_with(
        incorrect_plain_password, MOCKED_HASHED_PASSWORD
    )
...
async def test_authenticate_non_existent_username(db: AsyncSession):
    ...
    with mock.patch.object(core_security_module, "verify_password") as mock_verify:
        authenticated_user = await crud_user_instance.authenticate(
            db=db, username="no_such_auth_user", password="anypassword"
        )
...
def test_is_active():
    ...
    inactive_user = User(
        username="inactive_test", hashed_password="pw", is_active=False
    )
...
def test_is_superuser():
    ...
    regular_user = User(
        username="regular_test", hashed_password="pw", is_superuser=False
    )
