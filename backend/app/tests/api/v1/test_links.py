...
async def test_create_link(
    ...
    link_data = {
        ...
        "tags": "test,create",
        # 测试 cookies_path 字段 / Test cookies_path field
        "cookies_path": "path/to/cookies.txt",
        "settings": {"quality": "high"},
    }
    response = await client.post(
        f"{settings.API_V1_STR}/links/", json=link_data, headers=superuser_token_headers
    )
    assert response.status_code == 201
    created_link = response.json()
    assert created_link["url"] == url
    assert created_link["name"] == name
    # 基于 extract_site_name / Based on extract_site_name
    assert created_link["site_name"] == "Example"
...
@pytest.mark.asyncio
async def test_create_link_duplicate_url(
    client: httpx.AsyncClient, superuser_token_headers: Dict[str, str]
) -> None:
    """测试创建具有重复 URL 的链接"""
...
async def test_read_links(
    ...
    link1 = await create_test_link(
        client, superuser_token_headers, "https://example.com/link1", "Link 1"
    )
    link2 = await create_test_link(
        client, superuser_token_headers, "https://anothersite.org/link2", "Link 2"
    )

    response = await client.get(
        f"{settings.API_V1_STR}/links/", headers=superuser_token_headers
    )
    assert response.status_code == 200
    links = response.json()
    assert isinstance(links, list)
    # 可能有其他测试留下的链接 / Might have links left from other tests if scope changes
    assert len(links) >= 2
    # 检查是否包含我们刚创建的链接 / Check if it contains the links we just created
    link_ids = [link_item["id"] for link_item in links]
...
async def test_read_link(
    ...
    link = await create_test_link(
        ...
        "https://example.com/read_single",
        "Read Single",
    )
    link_id = link["id"]

    response = await client.get(
        f"{settings.API_V1_STR}/links/{link_id}", headers=superuser_token_headers
    )
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_read_link_not_found(
    client: httpx.AsyncClient, superuser_token_headers: Dict[str, str]
) -> None:
    """测试读取不存在的链接"""
    response = await client.get(
        f"{settings.API_V1_STR}/links/99999", headers=superuser_token_headers
    )
...
async def test_update_link(
    ...
    """测试更新链接"""
    link = await create_test_link(
        client, superuser_token_headers, "https://example.com/update_me", "Update Me"
    )
    link_id = link["id"]
    update_data = {
        "name": "Updated Name",
    ...
    response = await client.put(
        f"{settings.API_V1_STR}/links/{link_id}",
        json=update_data,
    ...
    # 检查更新时间 / Check updated timestamp
    assert updated_link["updated_at"] > link["updated_at"]
...
async def test_delete_link(
    ...
    """测试删除链接"""
    link = await create_test_link(
        client, superuser_token_headers, "https://example.com/delete_me", "Delete Me"
    )
    link_id = link["id"]
