...
async def create_test_link_for_history(
    client: httpx.AsyncClient, headers: Dict[str, str], url: str, name: str
) -> Dict:
    ...
    response = await client.post(
        f"{settings.API_V1_STR}/links/", json=link_data, headers=headers
    )
    assert (
        response.status_code == 201
    ), f"Failed to create link for history testing: {response.text}"
...
async def create_history_log_directly(
    ...
    history_log_create = HistoryLogCreate(
        link_id=link_id, status=status, output=output, timestamp=timestamp
    )
    history_log = HistoryLog.model_validate(
        history_log_create
...
@pytest.mark.asyncio
async def test_read_history_logs_empty(
    client: httpx.AsyncClient, superuser_token_headers: Dict[str, str]
) -> None:
    """Test reading history logs when none exist (initially)."""
    # Clear existing history logs (and links to be safe, as they might cascade
    # or be related)
    async with TestSessionFactory() as session:
        # More direct way to delete
        await session.execute(select(HistoryLog).delete())
        # Clear links to avoid FK issues or leftover data
        await session.execute(select(Link).delete())
        await session.commit()

    response = await client.get(
        f"{settings.API_V1_STR}/history/", headers=superuser_token_headers
    )
...
async def test_create_and_read_history_logs(
    ...
    link_dict = await create_test_link_for_history(
        ...
        "https://example.com/history_test_1",
        "History Test Link 1",
    )
    ...
    link_dict_2 = await create_test_link_for_history(
        client,
        superuser_token_headers,
        "https://example.com/history_test_2",
        "History Test Link 2",
    )
    ...
    async with TestSessionFactory() as session:
        # log1 was unused F841
        await create_history_log_directly(
            session, link_id_1, status="success", output="Log 1 for link 1"
        )
        log2 = await create_history_log_directly(
            session, link_id_1, status="failed", output="Log 2 for link 1"
        )
        log3 = await create_history_log_directly(
            session, link_id_2, status="success", output="Log 1 for link 2"
        )
    ...
    response_link1 = await client.get(
        f"{settings.API_V1_STR}/history/?link_id={link_id_1}",
    ...
    response_success = await client.get(
        f"{settings.API_V1_STR}/history/?status=success",
        headers=superuser_token_headers,
    )
    ...
    response_failed = await client.get(
        f"{settings.API_V1_STR}/history/?status=failed", headers=superuser_token_headers
    )
    ...
    response_link1_success = await client.get(
        f"{settings.API_V1_STR}/history/?link_id={link_id_1}&status=success",
        headers=superuser_token_headers,
    )
    ...
    response_limit1 = await client.get(
        f"{settings.API_V1_STR}/history/?limit=1", headers=superuser_token_headers
    )
    ...
    response_skip1_limit1 = await client.get(
        f"{settings.API_V1_STR}/history/?skip=1&limit=1",
        headers=superuser_token_headers,
    )
...
async def test_delete_history_log(
    ...
    link_dict = await create_test_link_for_history(
        client,
        superuser_token_headers,
        "https://example.com/history_delete_test",
        "History Delete Test Link",
    )
    link_id = link_dict["id"]
    async with TestSessionFactory() as session:
        history_log = await create_history_log_directly(
            session, link_id, output="Log to be deleted"
        )
    ...
    delete_response = await client.delete(
        f"{settings.API_V1_STR}/history/{history_log_id}",
        headers=superuser_token_headers,
    )
    ...
    response = await client.get(
        f"{settings.API_V1_STR}/history/?link_id={link_id}",
        headers=superuser_token_headers,
    )
    assert response.status_code == 200
...
async def test_delete_history_log_not_found(
    ...
    delete_response = await client.delete(
        f"{settings.API_V1_STR}/history/{non_existent_id}",
        headers=superuser_token_headers,
    )
...
async def test_delete_history_log_unauthorized(client: httpx.AsyncClient) -> None:
    ...
    # 123 is a placeholder ID
    response = await client.delete(f"{settings.API_V1_STR}/history/123")
...
async def test_delete_history_logs_by_link(
    ...
    link1_dict = await create_test_link_for_history(
        client,
        superuser_token_headers,
        "https://example.com/history_by_link_1",
        "History By Link 1",
    )
    ...
    link2_dict = await create_test_link_for_history(
        client,
        superuser_token_headers,
        "https://example.com/history_by_link_2",
        "History By Link 2",
    )
    ...
    async with TestSessionFactory() as session:
        await create_history_log_directly(
            session, link1_id, output="Log 1 for link 1 (by_link test)"
        )
        await create_history_log_directly(
            session, link1_id, output="Log 2 for link 1 (by_link test)"
        )
        await create_history_log_directly(
            session, link2_id, output="Log 1 for link 2 (by_link test)"
        )

    # 3. Delete history logs for link1_id
    delete_response = await client.delete(
        f"{settings.API_V1_STR}/history/by_link/{link1_id}",
        headers=superuser_token_headers,
    )
    assert delete_response.status_code == 200
    ...
    assert (
        ...
        == f"Successfully deleted 2 history logs for link_id {link1_id}"
    )

    # 4. Verify logs for link1_id are deleted
    response_link1 = await client.get(
        f"{settings.API_V1_STR}/history/?link_id={link1_id}",
        headers=superuser_token_headers,
    )
    assert response_link1.status_code == 200
    ...
    response_link2 = await client.get(
        f"{settings.API_V1_STR}/history/?link_id={link2_id}",
        headers=superuser_token_headers,
    )
...
async def test_delete_history_logs_by_link_link_not_found(
    ...
    delete_response = await client.delete(
        f"{settings.API_V1_STR}/history/by_link/{non_existent_link_id}",
        headers=superuser_token_headers,
    )
    # As per API endpoint, it checks if link exists
    assert delete_response.status_code == 404
    assert (
        f"Link with id {non_existent_link_id} not found"
...
async def test_delete_history_logs_by_link_no_history(
    ...
    link_dict = await create_test_link_for_history(
        client,
        superuser_token_headers,
        "https://example.com/history_no_logs",
        "No History Link",
    )
    link_id = link_dict["id"]
    ...
    delete_response = await client.delete(
        f"{settings.API_V1_STR}/history/by_link/{link_id}",
        headers=superuser_token_headers,
    )
    assert delete_response.status_code == 200
    response_data = delete_response.json()
    assert (
        response_data["message"]
        == f"Successfully deleted 0 history logs for link_id {link_id}"
    )

    # 3. Verify no logs are present (should still be empty)
    response_link = await client.get(
        f"{settings.API_V1_STR}/history/?link_id={link_id}",
        headers=superuser_token_headers,
    )
...
@pytest.mark.asyncio
async def test_delete_history_logs_by_link_unauthorized(
    client: httpx.AsyncClient,
) -> None:
    """Test deleting history logs by link_id without authentication."""
    response = await client.delete(
        f"{settings.API_V1_STR}/history/by_link/123"
    )  # 123 is a placeholder link_id
    assert response.status_code == 401
...
# For now, individual tests try to manage their data or test against empty
# states.
