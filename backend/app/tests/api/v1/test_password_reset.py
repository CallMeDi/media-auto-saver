...
# from app.crud import password_reset_token as crud_password_reset_token # Unused
from app.tests.conftest import TestSessionFactory  # Import test DB session factory
# from app.api.v1.endpoints.password_reset import ( # Unused
...
async def create_test_user_for_password_reset(
    ...
    # user_in = UserCreate(username=username, email=email, password=password) # Unused F841
    ...
    # db_user = await crud_user.create(db=db, obj_in=user_in) # If
    # crud_user.create is async and suitable

    # Simplified user creation for test setup if crud_user.create is complex
    # or sync-only
...
async def test_recover_password_generate_token_success(
    ...
    # superuser_token_headers: Dict[str, str] # Not needed for this public
    # endpoint
) -> None:
    ...
    async with TestSessionFactory() as session:
        await create_test_user_for_password_reset(
            session, test_username, test_email, test_password
        )

    response = await client.post(
        f"{settings.API_V1_STR}/password-recovery/{test_username}"
    )
    ...
    async with TestSessionFactory() as session:
        token_obj = await session.execute(
            select(PasswordResetToken).where(
                PasswordResetToken.token == reset_token_str
            )
        )
...
async def test_recover_password_generate_token_user_not_found(
    ...
    response = await client.post(
        f"{settings.API_V1_STR}/password-recovery/{non_existent_username}"
    )
...
async def test_reset_password_success(
    ...
    reset_response = await client.post(
        f"{settings.API_V1_STR}/reset-password/", json=reset_payload
    )
    ...
    async with TestSessionFactory() as session:
        user_after_reset = await crud_user.get_by_username(
            session, username=test_username
        )
        assert user_after_reset is not None
        assert (
            security.verify_password(new_password, user_after_reset.hashed_password)
            is True
        )
        assert (
            security.verify_password(
                original_password, user_after_reset.hashed_password
            )
            is False
        )
...
async def test_reset_password_used_token(
    ...
    reset_response1 = await client.post(
        f"{settings.API_V1_STR}/reset-password/", json=reset_payload1
    )
    ...
    reset_response2 = await client.post(
        f"{settings.API_V1_STR}/reset-password/", json=reset_payload2
    )
    assert reset_response2.status_code == 400
    assert (
        "invalid or has expired" in reset_response2.json()["detail"].lower()
    )  # API uses same message for used/expired
...
async def test_reset_password_new_password_too_short(
    ...
    # Expecting 422 Unprocessable Entity due to Pydantic validation on
    ...
    for error in error_details:
        if (
            error.get("loc")
            and "new_password" in error.get("loc")
            and "type" in error
            and "string_too_short" in error.get("type")
        ):
            found_password_error = True
            break
    assert (
        found_password_error
    ), "Did not find specific error for new_password being too short."
...
# for simplicity, assuming `crud_user.create` might have complexities not
# needed here.
print(
    "test_password_reset.py created with tests for password recovery and reset endpoints."
)
