...
for domain, site_name in KNOWN_SITES.items():
    ...
    # With www.
    known_sites_test_cases.append(
        (f"https://www.{domain}/another_page.html", site_name)
    )
...
unknown_sites_test_cases = [
    ...
    (
        "https://API.somedeveloper.DEV/v1/docs",
        "Somedeveloper",
    ),  # Test with uppercase TLD
...
edge_cases_test_cases = [
    ...
    # testsite.com is not in KNOWN_SITES
    ("https://testsite.com:1234/resource", "Testsite"),
    ...
    (
        "https://abc.org.cn/a/b",
        "Abc",
    ),  # Should take 'abc' as per current logic for unknown .org.cn
]
...
invalid_inputs_test_cases = [
    ...
    # URLs that urlparse might return an empty netloc for after stripping,
    # e.g., only scheme
    ("http://", None),
    ("https://", None),
]
