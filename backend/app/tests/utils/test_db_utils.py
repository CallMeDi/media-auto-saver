...
import pytest
import asyncio  # noqa: F401 Used for patching paths like app.utils.db_utils.asyncio
import os  # noqa: F401 Used for patching paths like app.utils.db_utils.os
import shutil  # noqa: F401 Used for patching paths like app.utils.db_utils.shutil
...
async def test_export_sqlite3_not_found(
    mock_shutil_which, mock_os_exists, mock_settings_db_url
):
...
@pytest.mark.asyncio
@mock.patch(
    "app.utils.db_utils.asyncio.create_subprocess_shell", new_callable=mock.AsyncMock
)
@mock.patch("app.utils.db_utils.shutil.which", return_value="/path/to/sqlite3")
# DB and output file (during removal check)
...
@mock.patch("app.utils.db_utils.shutil.which", return_value="/path/to/sqlite3")
# Dynamic side effect for os.path.exists
...
async def test_export_fails_process_error(
    ...
    mock_process.communicate = mock.AsyncMock(
        return_value=(b"", b"Error dumping database")
    )
    mock_subprocess_shell.return_value = mock_process

    output_sql_file = tmp_path / "failed_export.sql"

    # os.path.exists needs to return True for db, then True for output file
    # (to trigger remove)
...
async def test_export_exception_during_process(
    ...
    def os_exists_side_effect(path):
        ...
        if path == str(
            output_sql_file
        ):  # Assume file might have been created before exception
            return True
        return False
...
@pytest.mark.asyncio
@mock.patch(
    "app.utils.db_utils.os.path.exists", return_value=False
...
@mock.patch("app.utils.db_utils.shutil.which", return_value=None)  # sqlite3 not found
async def test_import_sqlite3_not_found(
    mock_shutil_which, mock_os_exists, mock_settings_db_url
):
...
@mock.patch("app.utils.db_utils.shutil.which", return_value="/path/to/sqlite3")
# Mock to control existence of SQL and DB files
@mock.patch("app.utils.db_utils.os.path.exists")
async def test_import_fail_to_delete_existing_db(
    ...
    def os_exists_side_effect(path):
        if path == str(sql_file):
            return True  # SQL file exists
        if path == db_path_from_settings:
            return True  # DB file exists (needs deletion)
        return False
...
async def test_import_successful(
    ...
    def os_exists_side_effect(path):
        if path == str(sql_file):
            return True  # SQL file exists
        if path == db_path_from_settings:
            return True  # Old DB exists (will be removed)
        return False
...
@mock.patch("app.utils.db_utils.shutil.which", return_value="/path/to/sqlite3")
@mock.patch(
    "app.utils.db_utils.os.path.exists", return_value=True
)  # SQL and DB files exist
async def test_import_fails_process_error(
    ...
    mock_process.returncode = 1
    mock_process.communicate = mock.AsyncMock(
        return_value=(b"", b"Error importing database")
    )
    mock_subprocess_shell.return_value = mock_process
