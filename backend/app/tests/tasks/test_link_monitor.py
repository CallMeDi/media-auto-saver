...
import pytest
import asyncio  # noqa: F401 Used for patching paths like app.tasks.link_monitor.asyncio
from unittest import mock
# from typing import List, Optional, Dict, Any # All unused
...
@pytest.mark.asyncio
@mock.patch(
    "app.tasks.link_monitor.crud.history_log.create_log", new_callable=mock.AsyncMock
)
@mock.patch("app.tasks.link_monitor.download_media", new_callable=mock.AsyncMock)
@mock.patch(
    "app.tasks.link_monitor.crud.link.update_status", new_callable=mock.AsyncMock
)
...
async def test_process_link_successful_download_creator(
    ...
    mock_crud_link_update_status.assert_any_call(
        db=mock_db_session,
        db_obj=mock_link,
    ...
    mock_crud_link_update_status.assert_any_call(
        db=mock_db_session, db_obj=mock_link, status=LinkStatus.IDLE, is_success=True
    )
...
async def test_process_link_exception_in_download_media(
    ...
    # The error message in the except block will be "Processing Exception:
    # Network Error"
    mock_crud_link_update_status.assert_any_call(
        db=mock_db_session,
        db_obj=mock_link,
...
async def test_process_link_exception_initial_crud_get(
    ...
    # Inside that, it tries crud.link.get again. Let's make that second one
    # also fail.

    # First call to crud.link.get raises, second call (in except block) also
    # raises
    mock_crud_link_get.side_effect = [
        Exception(exception_message),
        Exception("Second DB error"),
    ]
    ...
    mock_crud_link_update_status.assert_not_called()
    mock_download_media.assert_not_called()
    # History log also depends on link_for_status being found on the second
    # attempt.
    mock_crud_history_create_log.assert_not_called()
...
async def test_process_link_exception_initial_update_status(
    ...
    mock_crud_link_update_status.side_effect = [
        Exception(exception_message),
        None,
    ]  # Second call (in except) is fine

    await link_monitor.process_link(link_id)

    # crud.link.get called twice (once in main try, once in except block to
    # get link_for_status)
    assert mock_crud_link_get.call_count == 2
    ...
    # Should not be reached if initial update_status fails
    mock_download_media.assert_not_called()
...
@pytest.mark.asyncio
@mock.patch(
    "app.tasks.link_monitor.process_link", new_callable=mock.AsyncMock
)  # Mock the actual processing
@mock.patch("app.tasks.link_monitor.AsyncSessionFactory")
async def test_trigger_monitoring_job_no_enabled_links(
    mock_async_session_factory, mock_process_link_task  # Patched process_link
):
    mock_db_session = mock.AsyncMock()
    mock_async_session_factory.return_value.__aenter__.return_value = mock_db_session
    ...
    mock_logger_info.assert_any_call(
        "Scheduler triggered: Starting monitoring job for all enabled links..."
    )
    mock_logger_info.assert_any_call(
        "Scheduler job: No enabled and idle links found to process."
    )


@pytest.mark.asyncio
@mock.patch(
    "app.tasks.link_monitor.asyncio.gather", new_callable=mock.AsyncMock
...
async def test_trigger_monitoring_job_multiple_links(
    ...
    # link3 should be filtered out by the SQL query logic
    enabled_links_from_db = [link1, link2]
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = (
        enabled_links_from_db
    )

    # Mock create_task to just return a dummy task object (or a mock that can be awaited)
    # The important part is that process_link_with_semaphore is called within
    # create_task
    ...
    mock_logger_info.assert_any_call(
        f"Scheduler job: Created task for link_id: {link1.id} ({link1.url})"
    )
    mock_logger_info.assert_any_call(
        f"Scheduler job: Created task for link_id: {link2.id} ({link2.url})"
    )

    # Check if asyncio.gather was called with the created tasks
    mock_asyncio_gather.assert_called_once_with(
        *[dummy_task for _ in enabled_links_from_db]
    )

    mock_logger_info.assert_any_call(
        f"Scheduler job: Finished processing {len(enabled_links_from_db)} links."
    )
