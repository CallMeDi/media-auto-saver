...
# from .link import Link # 暂时不建立直接关系, 避免循环导入 / Avoid direct relationship
# for now to prevent circular imports
...
class HistoryLogBase(SQLModel):
    ...
    link_id: int = Field(
        foreign_key="link.id",
        index=True,
        description="关联的链接 ID / Associated Link ID",
    )
    # 中文: 使用 timezone.utc 确保时间戳是 aware 的 / Use timezone.utc to ensure
    # timestamp is aware
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        index=True,
        description="事件发生时间 / Event timestamp",
    )
    status: HistoryStatus = Field(
        description="任务状态 (成功/失败) / Task status (success/failure)"
    )
    ...
    downloaded_files: Optional[List[str]] = Field(
        ...
        description="下载/录制的文件列表 / List of downloaded/recorded files",
    )
    error_message: Optional[str] = Field(
        default=None, description="错误信息 (如果失败) / Error message (if failed)"
    )
    details: Optional[Dict[str, Any]] = Field(
        default=None,
        sa_column=Column(JSON),
        description="其他详细信息 (例如文件大小, 时长等) / Other details (e.g., file size, duration, etc.)",
    )
...
class HistoryLog(HistoryLogBase, table=True):
    ...
    # link: Optional["Link"] = Relationship(back_populates="history_logs") #
    # 需要在 Link 模型中添加 back_populates / Requires adding back_populates in Link
    # model
...
class HistoryLogRead(HistoryLogBase):
    ...
    id: int
    timestamp: (
        datetime  # 确保时间戳被正确序列化 / Ensure timestamp is serialized correctly
    )
