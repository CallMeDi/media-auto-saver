...
class LinkStatus(str, enum.Enum):
    ...
    # DISABLED = "disabled" # 已移除, 使用 is_enabled=False 代替 / Removed, use
...
class LinkBase(SQLModel):
    ...
    link_type: LinkType = Field(
        default=LinkType.CREATOR, description="链接类型 / Link type"
    )
    site_name: Optional[str] = Field(
        default=None,
        index=True,
        description="网站名称 (例如: Twitter, YouTube) / Site name (e.g., Twitter, YouTube)",
    )
    name: Optional[str] = Field(
        default=None,
        description="用户指定的名称或自动获取的名称 / User-specified or automatically fetched name",
    )
    description: Optional[str] = Field(
        default=None, description="用户添加的描述 / User-added description"
    )
    tags: Optional[str] = Field(
        default=None,
        description="用户添加的标签 (逗号分隔) / User-added tags (comma-separated)",
    )
    status: LinkStatus = Field(
        default=LinkStatus.IDLE, description="当前状态 / Current status"
    )
    last_checked_at: Optional[datetime] = Field(
        default=None, description="上次检查时间 / Last checked time"
    )
    last_success_at: Optional[datetime] = Field(
        default=None,
        description="上次成功下载/录制时间 / Last successful download/record time",
    )
    error_message: Optional[str] = Field(
        default=None, description="错误信息 / Error message"
    )
    # 中文: 存储特定于链接的设置 (例如下载路径模板, 录制质量等)
    # English: Store link-specific settings (e.g., download path template,
    # recording quality, etc.)
    settings: Dict[str, Any] = Field(
        default={},
        sa_column=Column(JSON),
        description="特定设置 (JSON) / Specific settings (JSON)",
    )
    ...
    cookies_path: Optional[str] = Field(
        default=None, description="Cookies 文件路径 / Cookies file path"
    )
    is_enabled: bool = Field(
        default=True, description="是否启用监控 / Whether monitoring is enabled"
    )
    ...
    updated_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="最后更新时间 / Last update time",
    )
...
class LinkUpdate(SQLModel):
    ...
    # 允许更新 Cookies 路径 / Allow updating cookies path
    cookies_path: Optional[str] = None
    is_enabled: Optional[bool] = None
    # link_type 和 site_name 通常不应被更新 / link_type and site_name should generally
    # not be updated
