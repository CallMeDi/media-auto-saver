...
# 导入 Column 和 DateTime / Import Column and DateTime
from sqlalchemy import Column, DateTime

# 用于获取令牌过期时间 / To get token expiration time
# from app.core.config import settings # Not used in this file

# 中文: 定义密码重置令牌的默认过期时间 (例如: 1 小时)
# English: Define default expiration time for password reset tokens (e.g.,
# 1 hour)
...
class PasswordResetTokenBase(SQLModel):
    ...
    user_id: int = Field(
        foreign_key="user.id",
        index=True,
        description="关联的用户 ID / Associated User ID",
    )
    # 中文: 明确指定数据库列类型为带时区的 DateTime
    # English: Explicitly specify the database column type as DateTime with
    # timezone
    expires_at: datetime = Field(
        sa_column=Column(DateTime(timezone=True)),
        description="令牌过期时间 / Token expiration time",
    )
    used: bool = Field(
        default=False, description="令牌是否已被使用 / Whether the token has been used"
    )
...
def calculate_expiry_date() -> datetime:
    ...
    )  # 确保返回 aware datetime / Ensure aware datetime is returned
