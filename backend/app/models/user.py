...
class UserBase(SQLModel):
    ...
    email: Optional[str] = Field(
        default=None, unique=True, index=True, description="邮箱 / Email"
    )
    full_name: Optional[str] = Field(default=None, description="全名 / Full name")
    is_active: bool = Field(
        default=True, description="用户是否激活 / Is the user active"
    )
    is_superuser: bool = Field(
        default=False, description="是否为超级用户 / Is superuser"
    )
