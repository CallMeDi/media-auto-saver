...
YDL_DEFAULT_OPTS = {
    # 优先下载 mp4 格式 / Prioritize mp4 format
    "format": "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best",
    # 输出路径模板 / Output path template
    ...
    # 进度回调 (可以用于更新状态) / Progress hooks (can be used for status updates)
    "progress_hooks": [],
    ...
    ),  # 下载记录文件, 避免重复下载 / Archive file to avoid re-downloading
...
# English: Define default options for gallery-dl (simulated via
# command-line arguments)
GDL_DEFAULT_ARGS = [
    "--directory",
    settings.MEDIA_ROOT,  # 下载目录 / Download directory
    "--write-metadata",  # 写入元数据文件 / Write metadata file
    # ugoira (动图) 转为无损格式 / Convert ugoira (animated images) to lossless format
    "--ugoira-conv-lossless",
    "--retries",
    "5",  # 重试次数 / Number of retries
    "--sleep",
    "1-3",  # 下载间隔随机睡眠 / Random sleep between downloads
    # 下载记录数据库 (修正参数名) / Archive database (corrected parameter name)
    "--download-archive",
    os.path.join(settings.MEDIA_ROOT, "gallery_dl_archive.sqlite"),
...
def get_downloader_for_link(link: Link) -> Tuple[str, Dict[str, Any] | List[str]]:
    ...
    if site in [
        ...
        if link.cookies_path:
            full_cookie_path = os.path.join(
                PROJECT_ROOT, USER_COOKIES_BASE_DIR_NAME, link.cookies_path
            )
            if os.path.exists(full_cookie_path):
                cookie_path_to_use = full_cookie_path
                logger.info(
                    f"Using link-specific cookies for link {link.id}: {cookie_path_to_use}"
                )
            else:
                logger.warning(
        ...
        if not cookie_path_to_use:
            ...
            if global_cookie_path and os.path.exists(global_cookie_path):
                ...
                logger.info(
                    f"Using global cookies for site '{site}': {cookie_path_to_use}"
                )
            elif global_cookie_path:
                logger.warning(
                    f"Global cookies file specified for site '{site}' but not found at: {global_cookie_path}"
                )

        if cookie_path_to_use:
            gdl_args.extend(["--cookies", cookie_path_to_use])

        # TODO: 可以根据 link.settings 添加特定参数 / TODO: Add specific arguments based
        # on link.settings
    ...
    if link_type == LinkType.LIVE:
        ydl_opts["outtmpl"] = os.path.join(
            ...
            "Live Recordings",
            "%(uploader)s [%(channel_id)s]",
            "%(title)s - %(timestamp)s [%(id)s].%(ext)s",
        )
    ...
    # English: Check if yt-dlp also needs cookies (prioritize link-specific,
    # then global)
    cookie_path_to_use_ydl = None
    if link.cookies_path:
        full_cookie_path_ydl = os.path.join(
            PROJECT_ROOT, USER_COOKIES_BASE_DIR_NAME, link.cookies_path
        )
        if os.path.exists(full_cookie_path_ydl):
            cookie_path_to_use_ydl = full_cookie_path_ydl
            logger.info(
                f"Using link-specific cookies for link {link.id} (yt-dlp): {cookie_path_to_use_ydl}"
            )
        else:
            logger.warning(
    ...
    if not cookie_path_to_use_ydl:
        ...
        if global_cookie_path_ydl and os.path.exists(global_cookie_path_ydl):
            ...
            logger.info(
                f"Using global cookies for site '{site}' (yt-dlp): {cookie_path_to_use_ydl}"
            )
        elif global_cookie_path_ydl:
            logger.warning(
                f"Global cookies file specified for site '{site}' (yt-dlp) but not found at: {global_cookie_path_ydl}"
            )
    ...
    # Example: if 'output_template' in link.settings: ydl_opts['outtmpl'] =
    # link.settings['output_template']
...
async def download_media(link: Link) -> Dict[str, Any]:
    ...
    try:
        if downloader_name == "yt-dlp":
            ...
            def ydl_filename_hook(d):
                if d["status"] == "finished":
                    ...
                    # Prioritize filepath from info_dict, this is the final
                    # processed path
                    filepath = d.get("info_dict", {}).get("filepath")
                    if not filepath:
                        # 如果 info_dict 中没有, 尝试使用顶层的 filename (可能是原始或临时名)
                        # If not in info_dict, try top-level filename (might be
                        # original or temp name)
                    ...
                    if filepath:
                        ...
                        if (
                            not filepath.endswith((".part", ".temp", ".ytdl"))
                            and os.path.exists(filepath)
                            and os.path.isfile(filepath)
                        ):
                            logger.debug(
                                f"yt-dlp hook: Detected finished file: {filepath}"
                            )
                            downloaded_files_list.append(filepath)
                        else:
                            logger.debug(
                                f"yt-dlp hook: Ignoring potential temp file or non-existent file: {filepath}"
                            )
                    else:
                        logger.debug(
                            f"yt-dlp hook: No filepath found in finished status dict: {d}"
                        )

            # 将钩子添加到选项中 / Add hook to options
            # 注意: progress_hooks 也可以用来获取信息, 但 postprocessor_hooks 更适合获取最终文件
            # Note: progress_hooks can also get info, but postprocessor_hooks
            # are better for final files
            # 使用 progress hook 更通用 / Using progress hook is more general
            ydl_opts["progress_hooks"] = [ydl_filename_hook]
            # --------------------------

            # 中文: 确保输出目录存在 (yt-dlp 通常会自动创建, 但以防万一)
            # English: Ensure output directory exists (yt-dlp usually creates
            # it, but just in case)
            output_template = ydl_opts.get("outtmpl", YDL_DEFAULT_OPTS["outtmpl"])
            # 提取目录部分 / Extract directory part
            ...
            # 实际目录由 yt-dlp 在下载时根据信息创建 / Actual directory is created by yt-dlp
            # during download based on info
            ...
            # 选项可能过长, 不打印 / Options might be too long, don't print
            logger.info(f"Starting yt-dlp download for {link.url}")
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    # download() 返回 0 表示成功, 非 0 表示有错误 (但 ignoreerrors=True 时仍可能下载了部分文件)
                    # download() returns 0 on success, non-zero indicates
                    # errors (but with ignoreerrors=True, some files might
                    # still be downloaded)
                    ...
                    # If the hook found files, assume success for the link,
                    # otherwise check for errors.
                    if downloaded_files_list:
                        result["status"] = "success"
                        result["downloaded_files"] = list(
                        ...
                        result["status"] = "error"
                        logger.warning(
                            f"yt-dlp download finished for {link.url}. Status: error (no files detected)."
                        )

            except yt_dlp.utils.DownloadError as de:
                # Catch specific download errors (even with ignoreerrors=True,
                # some critical errors are raised)
                logger.error(
                    f"yt-dlp DownloadError for {link.url}: {de}", exc_info=False
                )
                result["error"] = f"yt-dlp DownloadError: {de}"
                result["status"] = "error"
                # Even on error, check if some files were downloaded by the
                # hook
                if downloaded_files_list:
                    ...
                    logger.error(
                        f"yt-dlp download failed for {link.url} with DownloadError."
                    )
            ...
            logger.info(
                f"Starting gallery-dl download for {link.url} with args: {full_args}"
            )
            ...
            try:
                ...
                # Regex: Match paths starting with MEDIA_ROOT, possibly quoted,
                # ignore leading log prefix
                path_pattern = re.compile(
                    # 可选的日志前缀 / Optional log prefix
                    r"^(?:\[[^\]]+\]\s*)*"
                    # 路径部分, 可能被引号包围 / Path part, possibly quoted
                    # 可选的 "Writing data to" / Optional "Writing data to"
                    r"(?:Writing data to\s*)?"
                    r"['\"]?(" + re.escape(settings.MEDIA_ROOT) + r"/[^'\"\s]+)['\"]?"
                )

                for line in stdout.splitlines():
                    # 移除首尾空格 / Remove leading/trailing whitespace
                    ...
                    if match:
                        ...
                        if os.path.exists(filepath) and os.path.isfile(filepath):
                            logger.debug(
                                f"gallery-dl parser: Found potential file: {filepath}"
                            )
                            downloaded_files_list.append(filepath)
                        else:
                            logger.debug(
                                f"gallery-dl parser: Ignoring non-existent or non-file path: {filepath}"
                            )
                ...
                if process.returncode == 0:
                    ...
                    if not result["downloaded_files"]:
                        logger.warning(
                            f"gallery-dl for {link.url} finished successfully but no files were parsed from output."
                        )
                    # 只记录部分输出 / Log partial output
                    logger.info(f"gallery-dl stdout for {link.url}:\n{stdout[:500]}...")
                else:
                    result["error"] = (
                        f"gallery-dl failed with code {process.returncode}. Stderr: {stderr}"
                    )
                    result["status"] = "error"
                    # 即使失败, 也记录可能已下载的文件 / Even on failure, log potentially
                    # downloaded files
                    result["downloaded_files"] = list(set(downloaded_files_list))
                ...
                logger.error(
                    "gallery-dl command not found. Please ensure gallery-dl is installed and in PATH."
                )
                result["error"] = "gallery-dl command not found."
                result["status"] = "error"
            except Exception as e:
                logger.error(
                    f"Error running gallery-dl for {link.url}: {e}", exc_info=True
                )
        ...
        logger.error(
            f"Unexpected error during download preparation for {link.url}: {e}",
            exc_info=True,
        )
    ...
    # Ensure the collected file list is returned even if unexpected errors
    # occur
...
if __name__ == "__main__":
    ...
    # import asyncio # Redundant, already imported globally

    # asyncio.run(test()) # 取消注释以运行测试 / Uncomment to run the test
    print(
        "Downloader service module loaded. Run with test() function for basic checks."
    )
