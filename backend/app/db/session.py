...
# allowing multiple threads to access the same connection (FastAPI runs
# routes in a background thread pool)
async_engine = create_async_engine(
    settings.DATABASE_URL,
    # 中文: 设置为 True 可以打印 SQL 语句 (用于调试) / English: Set to True to print SQL
...
AsyncSessionFactory = async_sessionmaker(
    bind=async_engine,
    # 中文: 防止在提交后访问对象时需要重新查询 / English: Prevent needing to re-query objects
    # after commit
