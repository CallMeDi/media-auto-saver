...
# Removed re-imported block
from dotenv import set_key  # For reading/writing .env

from app import models
from app.api import deps

# 导入 settings 和项目根目录 / Import settings and project root
...
class SiteCookiesUpdate(BaseModel):
    site_cookies: Dict[str, str] = Field(
        ..., description="站点 Cookies 映射 / Site cookies mapping"
    )
...
async def get_global_site_cookies(
    ...
    # Note: Directly returns value from settings. More robust approach might
    # read from DB or dedicated config file.
...
async def update_global_site_cookies(
    *,
    ...
    # 限制只有管理员能修改 / Restrict modification to superusers
) -> Any:
    """
    ...
    try:
        ...
        # This handles creating the file if it doesn't exist and preserves
        # other variables/comments
        set_key(
            dotenv_path=env_path,
            key_to_set="SITE_COOKIES_JSON",
            value_to_set=cookies_json_str,
            quote_mode="always",
        )
        ...
        logger.error(
            f"Failed to persist site cookies to {env_path}: {e}", exc_info=True
        )
        raise HTTPException(
