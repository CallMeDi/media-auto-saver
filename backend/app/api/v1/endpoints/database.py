...
# 导入 BackgroundTask / Import BackgroundTask
...
# from app import models # No longer used directly
...
async def export_db(
    ...
    # 获取当前用户 (如果需要记录操作者) / Get current user (if operator logging is needed)
):
    """
    ...
    # Using NamedTemporaryFile and handling deletion in FileResponse might be
    # more elegant
    ...
    # English: Use FileResponse to send the file, and add a background task to
    # delete the temporary file
    return FileResponse(
        path=export_filename,
        filename=f"media_auto_saver_backup_{timestamp}.sql",
        media_type="application/sql",
        # 添加后台任务删除文件 / Add background task to delete file
...
async def import_db(
    ...
    file: UploadFile = File(
        ..., description="上传 SQL 文件进行导入 / Upload SQL file for import"
    ),
    # current_user: models.User = Depends(deps.get_current_active_user)
):
    """
    ...
    if not file.filename.endswith(".sql"):
        raise HTTPException(
            status_code=400, detail="Invalid file type. Please upload a .sql file."
        )
    ...
    # English: Perform the import operation in a background task and delete
    # the temp file afterwards
    background_tasks.add_task(run_import_and_cleanup, temp_filepath)

    return {
        "message": "Database import process started in the background. "
...
async def run_import_and_cleanup(temp_filepath: str):
    ...
    if success:
        ...
        # English: After success, might need to notify admin or trigger app
        # reload (if uvicorn --reload isn't sufficient)
    else:
    ...
    try:
        ...
        logger.error(
            f"Background task: Failed to delete temporary import file {temp_filepath}: {e}"
        )
    ...
    logger.warning(
        "Database import finished. Application restart might be required for changes to fully apply."
    )
