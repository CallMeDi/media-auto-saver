...
from app import crud  # 导入 models / Import models

# 导入 HistoryLog 模型 / Import HistoryLog model
from app.models.history import HistoryLog, HistoryLogRead
...
@router.get(
    "/",
    response_model=List[HistoryLogRead],
...
async def read_history_logs(
    ...
    skip: int = 0,
    limit: int = 100,
    link_id: Optional[int] = Query(
        None, description="按关联的链接 ID 过滤 / Filter by associated link ID"
    ),
    # 添加状态过滤参数 / Add status filter parameter
    status: Optional[str] = Query(
        None,
        description="按状态过滤 (success 或 failed) / Filter by status (success or failed)",
    ),
) -> Any:
    """
    ...
    if status is not None:
        ...
        # Ensure status is valid (frontend dropdown limits, but backend should
        # also validate)
        # 根据 HistoryLog 模型中的实际状态调整 / Adjust based on actual statuses in
        # HistoryLog model
...
async def delete_history_logs_by_link(
    *,
    ...
    return {
        "message": f"Successfully deleted {deleted_count} history logs for link_id {link_id}"
    }
