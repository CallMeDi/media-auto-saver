...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get_multi(
        ...
        *,
        skip: int = 0,
        limit: int = 100,
        # 新增排序参数 / Added order_by parameter
    ...
    async def update(
        ...
        *,
        ...
        if isinstance(obj_in, dict):
            ...
            # English: Use Pydantic model's model_dump method,
            # exclude_unset=True means only include explicitly set fields
            update_data = obj_in.model_dump(exclude_unset=True)

        # 中文: 遍历更新数据, 更新数据库对象的字段
        # English: Iterate through update data and update the database object's
        # fields
        ...
        if hasattr(db_obj, "updated_at"):
            # 使用 aware datetime / Use aware datetime
...
class CRUDLink(CRUDBase[Link, LinkCreate, LinkUpdate]):
    ...
    def _validate_and_normalize_cookies_path(
        self, user_path: Optional[str]
    ) -> Optional[str]:
        if not user_path:
            return None
        ...
        if os.path.isabs(user_path):
            raise ValueError(
                "cookies_path must be a relative path, not an absolute path."
            )

        # Normalize the user-provided path (e.g., collapses ../, ./, normalizes slashes)
        # Treat user_path as a filename or path relative to
        # USER_COOKIES_BASE_DIR_NAME
        normalized_filename = os.path.normpath(user_path)

        # After normalization, check for attempts to go outside the intended
        # directory
        if (
        ...
        full_path_to_check = os.path.normpath(
            full_path_to_check
        )  # Normalize again after join

        # Final security check: ensure the resolved path is actually within
        # cookies_base_dir
        if os.path.commonpath([cookies_base_dir]) != os.path.commonpath(
            [cookies_base_dir, full_path_to_check]
        ):
            raise ValueError(
                "cookies_path resolves outside the designated cookies directory."
            )
        ...
        if not os.path.isfile(full_path_to_check):
            raise ValueError(
                f"Specified cookies_path is not a file: {normalized_filename}"
            )

        # Return the clean, normalized filename/relative path to be stored in
        # DB
    ...
    async def create(self, db: AsyncSession, *, obj_in: LinkCreate) -> Link:
        ...
        if "cookies_path" in obj_in_data and obj_in_data["cookies_path"] is not None:
            try:
                obj_in_data["cookies_path"] = self._validate_and_normalize_cookies_path(
                    obj_in_data["cookies_path"]
                )
            except ValueError as e:
                # This exception should be caught by the API layer to return
                # HTTP 400
                raise e
    ...
    async def update(
        self, db: AsyncSession, *, db_obj: Link, obj_in: LinkUpdate | dict[str, Any]
    ) -> Link:
        if isinstance(obj_in, dict):
            update_data = obj_in
        else:
        ...
        if "cookies_path" in update_data:  # Handles explicit None to clear path too
            if update_data["cookies_path"] is not None:
                try:
                    update_data["cookies_path"] = (
                        self._validate_and_normalize_cookies_path(
                            update_data["cookies_path"]
                        )
                    )
                except ValueError as e:
                    # This exception should be caught by the API layer to
                    # return HTTP 400
                    raise e
            # If update_data["cookies_path"] is None, it will be set to None,
            # effectively clearing the path

        # The rest of the original update method from CRUDBase
        # Convert the database object to a dictionary
        # This line was missing from the provided snippet for update, adding it
        # back from CRUDBase
        obj_data = db_obj.model_dump()
        for field in obj_data:
            if field in update_data:
    ...
    async def get_enabled_links(
        self, db: AsyncSession, *, link_type: Optional[str] = None
    ) -> List[Link]:
        """
        中文: 获取所有启用的链接, 可选按类型过滤。
        English: Get all enabled links, optionally filtered by type.
        """
        query = select(Link).where(Link.is_enabled)
        if link_type:
    ...
    async def update_status(
        ...
        *,
        ...
        # 新增参数, 指示操作是否成功 / Added parameter to indicate if the operation was
        # successful
        ...
        update_data = {
            "status": status,
            # 总是更新检查时间 / Always update check time
