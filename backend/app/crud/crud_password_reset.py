...
class CRUDPasswordResetToken(
    CRUDBase[PasswordResetToken, PasswordResetTokenCreate, SQLModel]
):  # UpdateSchema 未使用 / UpdateSchema unused
    """
    ...
    async def create_reset_token(
        ...
        token_obj = PasswordResetToken(
            token=token, user_id=user_id, expires_at=expires_at, used=False
        )
        db.add(token_obj)
        await db.commit()
    ...
    async def use_token(
        ...
        """
        token_obj.used = True
        # 使其立即过期, 使用 aware datetime / Make it expire immediately, use aware
        # datetime
    ...
    def is_token_valid(self, token_obj: PasswordResetToken) -> bool:
        ...
        # English: If expires_at read from DB is naive, make it aware (assume
        # UTC)
