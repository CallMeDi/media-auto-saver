...
# Define constant for user cookies directory name
...
class Settings(BaseSettings):
    ...
    # Default value: Create an SQLite file named database.db in the project
    # root directory
    DATABASE_URL: str = (
        f"sqlite+aiosqlite:///{os.path.join(PROJECT_ROOT, 'database.db')}"
    )

    # 中文: 媒体文件下载/录制的根目录
    # English: Root directory for downloaded/recorded media files
    # 默认值: 在项目根目录下创建一个名为 media 的目录
    # Default value: Create a directory named media in the project root
    # directory
    ...
    # English: JWT related settings (if authentication is needed) - leave
    # blank for now
    SECRET_KEY: Optional[str] = None
    ACCESS_TOKEN_EXPIRE_MINUTES: int = 60 * 24 * 8  # 8 days

    # 中文: 初始超级用户凭证 (建议通过环境变量设置)
    # English: Initial superuser credentials (recommended to set via
    ...
    # Can be set via environment variable SITE_COOKIES_JSON='{"weibo":
    # "path/to/weibo.txt", "pixiv": "path/to/pixiv.txt"}'
    ...
    LINK_MONITOR_INTERVAL_MINUTES: int = int(
        os.getenv("LINK_MONITOR_INTERVAL_MINUTES", "60")
    )

    # 中文: 使用 ConfigDict 替代 class Config / English: Use ConfigDict instead of
    # class Config
    model_config = {
        "env_file": os.path.join(PROJECT_ROOT, ".env"),
        "env_file_encoding": "utf-8",
        # 忽略 .env 文件中未在模型中定义的额外变量 / Ignore extra variables in .env not defined
        # in the model
        "extra": "ignore",
    }
...
# English: Create an instance of Settings, settings will be loaded when
...
if __name__ == "__main__":
    ...
    print(
        f"Database will be stored at: {settings.DATABASE_URL.replace('sqlite+aiosqlite:///', '')}"
    )
