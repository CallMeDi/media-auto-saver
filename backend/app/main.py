...
from app.api.v1.api import (
    api_router as api_v1_router,
)  # 导入 v1 路由 / Import v1 router
from app.core.config import settings  # 导入配置 / Import settings

# 导入日志配置函数 / Import logging configuration function
from app.core.logging_config import setup_logging

# 导入数据库初始化函数和会话工厂 / Import DB init function and session factory
from app.db.session import init_db, AsyncSessionFactory

# 导入调度器 / Import scheduler
from app.tasks.scheduler import scheduler, start_scheduler, shutdown_scheduler

# 导入监控任务 / Import monitoring job
from app.tasks.link_monitor import trigger_monitoring_job

# 导入链接模型和状态 / Import Link model and status
from app.models.link import Link, LinkStatus
from app import (
    crud,
    models,
)  # 导入 CRUD 操作和 models / Import CRUD operations and models
...
# English: Define an asynchronous context manager to handle application
...
async def lifespan(app: FastAPI):
    ...
    async with AsyncSessionFactory() as db:
        ...
        query = select(models.User).where(models.User.is_superuser).limit(1)
        ...
        if not superuser:
            ...
            # 强烈建议修改! / Strongly recommend changing this!
            initial_password = settings.INITIAL_SUPERUSER_PASSWORD or "changeme"
            # initial_email = settings.INITIAL_SUPERUSER_EMAIL or "admin@example.com"

            # 检查是否使用了默认密码 (如果密码是从 settings 加载的, 即使是 'changeme', 也认为已设置)
            # Check if the default password is being used (if loaded from
            # settings, even 'changeme' is considered set)
            ...
            if settings.INITIAL_SUPERUSER_USERNAME is None or using_default_password:
                logger.warning(
                    f"INITIAL_SUPERUSER_USERNAME or INITIAL_SUPERUSER_PASSWORD not set in environment/config. Using defaults."
                )
                logger.warning(
                    f"Default superuser username: '{initial_username}', password: '{'changeme' if using_default_password else '********'}'"
                )
            else:
                logger.info(
                    "Using initial superuser credentials from environment/config."
                )
            ...
            user_in = models.UserCreate(
                ...
                # 使用从 settings 或默认值获取的密码 / Use password from settings or default
                # email=initial_email,
                is_superuser=True,
            ...
            try:
                ...
                logger.info(
                    f"Initial superuser '{initial_username}' created successfully."
                )
            except Exception as e:
                logger.error(
                    f"Failed to create initial superuser '{initial_username}': {e}",
                    exc_info=True,
                )
        else:
    ...
    async with AsyncSessionFactory() as db:
        ...
        for link in links_to_reset:
            ...
            await crud.link.update_status(
                ...
                error_message="Reset on startup",
            )
            reset_count += 1
        if reset_count > 0:
    ...
    logger.info(
        f"Scheduling link monitoring job with interval: {settings.LINK_MONITOR_INTERVAL_MINUTES} minutes."
    )
    scheduler.add_job(
        trigger_monitoring_job,
        "interval",
        # 从配置读取间隔 / Read interval from config
        minutes=settings.LINK_MONITOR_INTERVAL_MINUTES,
...
app = FastAPI(
    ...
    # OpenAPI schema 路径 / OpenAPI schema path
    lifespan=lifespan,
)
...
async def read_root():
    ...
    return {
        "message": "欢迎使用 Media Auto Saver API / Welcome to Media Auto Saver API"
    }


# 中文: 在脚本直接运行时, 启动 uvicorn 服务器 (主要用于开发)
# English: When the script is run directly, start the uvicorn server
# (mainly for development)
if __name__ == "__main__":
    import uvicorn

    # 中文: 运行 uvicorn 服务器, 监听 0.0.0.0:8000, 开启热重载
    # English: Run the uvicorn server, listen on 0.0.0.0:8000, enable hot
    # reload
