...
async def export_database_to_sql(output_filename: str) -> bool:
    ...
    if not sqlite3_cmd:
        logger.error(
            "sqlite3 command not found. Please install SQLite command-line tools."
        )
        # 或者可以尝试使用 Python 内置的 sqlite3 模块的 iterdump() 方法作为备选
        # Alternatively, try using the iterdump() method of Python's built-in
        # sqlite3 module as a fallback
        return False
    ...
    try:
        ...
        # English: Use asyncio.create_subprocess_shell to execute the command
        ...
        if process.returncode == 0:
            ...
            logger.error(
                f"Database export failed. Return code: {process.returncode}. Error: {error_message}"
            )
            # 中文: 如果失败, 删除可能已创建的不完整文件
            # English: If failed, delete the potentially created incomplete
            # file
...
async def import_database_from_sql(sql_filepath: str) -> bool:
    ...
    if os.path.exists(db_path):
        try:
            ...
            logger.error(
                f"Failed to delete existing database file {db_path}: {e}", exc_info=True
            )
            return False
    ...
    # 不记录完整命令以防文件路径敏感 / Don't log full command in case file path is sensitive
    logger.info("Executing database import command...")
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            # 这里我们假设调用者会处理后续事宜 / Here we assume the caller handles subsequent
            # actions
            return True
        else:
            error_message = stderr.decode().strip() if stderr else "Unknown error"
            logger.error(
                f"Database import failed. Return code: {process.returncode}. Error: {error_message}"
            )
            # 中文: 导入失败后, 数据库文件可能处于不一致状态 / After import failure, DB file might
...
if __name__ == "__main__":
    ...
    async def test_db_utils():
        ...
        export_file = os.path.join(
            settings.MEDIA_ROOT, f"db_export_test_{timestamp}.sql"
        )
